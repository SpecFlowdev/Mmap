...
- **Четыре вида схемы, переключаются одной кнопкой:**
  - **Поток** (по умолчанию) — отправители слева, кошелёк в центре, получатели справа.
    Карточки одинаковой высоты с долей в процентах, связи — тонкие кривые с градиентом,
    толщина по сумме, поэтому сразу видно, **откуда пришла основная масса денег и куда она ушла**.
    В центральном узле итоги «получено» и «отправлено».
  - **Майндмап** — дерево слева направо: кошелёк → группы → контрагенты.
